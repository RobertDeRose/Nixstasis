...
- **FR-001**: System MUST provide a single supported server deployment model for this migration and stop presenting legacy server package delivery as the default deployment path.
...
- **FR-013**: System MUST update release and deployment guidance so users can identify which legacy assets remain transitional and which assets are the new source of truth.
...
- Some legacy packaging artifacts may remain temporarily in the repository during transition, but they will no longer be presented as the supported server deployment path.
