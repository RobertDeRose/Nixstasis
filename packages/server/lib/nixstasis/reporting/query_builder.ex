defmodule Nixstasis.Reporting.QueryBuilder do
  ...
  defp query_filters_for_result_view(opts, fields) do
    ...
    |> Enum.map(fn filter ->
      target =
        filter["path"] || filter[:path] || filter["column"] || filter[:column] || filter["field"] || filter[:field]

      configured_path = field_path_for_column(target, fields)

      %{
        "path" => configured_path || filter["path"] || filter[:path] || filter["column"] || filter[:column],
        "field" => if(configured_path, do: nil, else: filter["field"] || filter[:field]),
  ...
  defp pushdown_filter?(filter) do
    ...
    (filter["column"] || filter[:column] || filter["field"] || filter[:field] || filter["path"] || filter[:path]) not in [
      nil,
      ""
    ] and
