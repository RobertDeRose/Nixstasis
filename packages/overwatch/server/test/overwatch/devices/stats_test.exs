defmodule Nixstasis.Devices.StatsTest do
  ...
  describe "dashboard stats" do
    test "count_all/0 returns correct count" do
      ...
      {:ok, _} = Devices.register_device(%{mac_address: "AA", product_key: "K"})
      {:ok, _} = Devices.register_device(%{mac_address: "BB", product_key: "K"})
    ...
    test "count_by_status/1 counts online/offline devices" do
      ...
      {:ok, online} = Devices.register_device(%{mac_address: "ON", product_key: "K"})
      {:ok, _} = Devices.update_last_seen(online)

      # Offline device (seen 10 mins ago)
      {:ok, offline} = Devices.register_device(%{mac_address: "OFF", product_key: "K"})
      ...
      {:ok, _never} = Devices.register_device(%{mac_address: "NEVER", product_key: "K"})
    ...
    test "count_pending_approvals/0 counts pending devices" do
      ...
      {:ok, app} = Devices.register_device(%{mac_address: "A1", product_key: "K"})
