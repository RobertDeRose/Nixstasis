...
func pollOnce(ctx context.Context, cfg *config.Config, client pollClient, executor scriptRunner, frpManager frpController, cmdHandler commandExecutor, uuid string, startTime time.Time, state *remoteAccessPollState) error {
	...
	if resp.RemoteAccessToken != "" {
		...
		} else if state != nil && state.tokenHash != "" && state.tokenHash != remoteAccessTokenHash {
		...
		} else if state != nil && state.tokenHash == "" {
...
func startFRP(frpManager frpController, cfg *config.Config, mac string, authToken string, authTokenHash string, state *remoteAccessPollState) {
