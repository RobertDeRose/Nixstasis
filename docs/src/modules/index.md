# Modules

This section documents repository modules by runtime context.

## Server Modules

- [Server Application](server-application.md)
- [Server Domain](server-domain.md)
- [Server Web](server-web.md)
- [Server Devices](server-devices.md)
- [Server Monitoring](server-monitoring.md)
- [Server Reporting](server-reporting.md)
- [Server E2E](server-e2e.md)

## Client Modules

- [Client CLI](client-cli.md)
- [Client Transport](client-transport.md)
- [Client Identity](client-identity.md)
- [Client Starlark Runtime](client-starlark-runtime.md)
- [Client Command Handler](client-command-handler.md)
- [Client FRP Manager](client-frp-manager.md)
- [Client E2E Harness](client-e2e-harness.md)

## Infrastructure Modules

- [Edge Caddy](edge-caddy.md)
- [Edge FRP](edge-frp.md)
- [Deployment Compose](deployment-compose.md)
- [Shared E2E Log Viewer](shared-e2e-log-viewer.md)
