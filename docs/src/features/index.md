# Implemented Features

These records describe delivered behavior, its integration with Nixstasis architecture, validation evidence, and the
historical decisions needed to audit each feature. Active and future work remains in
[Planned Features](../planned-features.md) and Beads.

## Feature Index

<!-- BEGIN IMPLEMENTED FEATURES -->
- [Add Rule Modal Improvements](add-rule-modal-improvements/index.md)
- [Ash API Contract Unification](ash-api-contract-unification/index.md)
- [AuthCrunch Role Contract](authcrunch-role-contract/index.md)
- [Compose Dev Harness](compose-dev-harness/index.md)
- [Dashboard Device Groups](dashboard-device-groups/index.md)
- [Dashboard Home](dashboard-home/index.md)
- [Device Detail Page](device-detail-page/index.md)
- [Go Client Rewrite](go-client-rewrite/index.md)
- [In-Memory SSH Authorized Keys](in-memory-ssh-authorized-keys/index.md)
- [IoT Device Monitoring](iot-device-monitoring/index.md)
- [Packaging and Deployment Migration](packaging-deployment-migration/index.md)
- [Phoenix UI Polish](phoenix-ui-polish/index.md)
- [Production Operations Runbooks](production-operations-runbooks/index.md)
- [Report View Improvements](report-view-improvements/index.md)
- [Rich API Examples](rich-api-examples/index.md)
- [Self-Extracting Installer](self-extracting-installer/index.md)
- [Server-Client E2E Tests](server-client-e2e-tests/index.md)
- [Server Command Allowlist Management](server-command-allowlist-management/index.md)
- [Server Curated Command Package Catalog](server-curated-command-package-catalog/index.md)
- [Server Stary Script Workbench](server-stary-script-workbench/index.md)
- [Server-Provided FRPS Token](server-provided-frps-token/index.md)
- [Starlark Script System](starlark-script-system/index.md)
<!-- END IMPLEMENTED FEATURES -->
