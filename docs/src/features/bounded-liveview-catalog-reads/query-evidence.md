...
The focused catalog suites recorded 36 passing tests with zero failures after the
strict source-validation and raw-cardinality fixes. `bounded_catalog_query_evidence_test.exs` adds executable query-count, returned-row,
...
| Manual policy preview  | Resolver tests exercise selected-entry/category scoping and the 10,000 source-row / 2,500 distinct-command preflight.                                                   | `bounded_catalog_query_evidence_test.exs` calls both actual preview functions: manual preview records 5 queries, 544 external-term bytes; catalog preview records 5 queries, 181 bytes. Actual manual and catalog preflight calls each insert 10,001 rows, return the source-row-limit error, and materialize only 92 external-term bytes; resolver tests assert no partial preview/queue result. | Narrow entry/source projections and explicit bound errors prevent oversized materialized payloads; actual accepted manual preview measured 544 bytes and rejected manual preflight measured 92 bytes.                                           |
| Catalog policy preview | Catalog resolver tests exercise selected command/category/device scoping, mapping joins, and the 2,500 distinct-command guard.                                          | The actual catalog preview records 5 repo queries after reusing validated direct IDs; the combined 10,001-row preflight records 3 manual-validation queries before the catalog resolver rejects the raw list, and no catalog command/mapping/device/snapshot payload is loaded after rejection. Direct catalog preview of an oversized raw list fails before any SQL query.                       | Selected command/mapping/device/snapshot fields are narrow; actual accepted catalog preview measured 181 bytes and rejected catalog preflight measured 92 bytes, while over-limit tests assert an explicit error rather than a partial payload. |
