...
- Do not create a merge commit when landing the integration branch in the default branch.
- Do not use `--no-ff`.
...
4. Verify the final target branch is the default branch.
...
- run `git fetch origin <default-branch> --tags`
...
1. Check whether the default branch has diverged from the integration branch base.
2. If the default branch has moved, rebase the integration branch onto the current default branch, resolve
...
5. Verify the default branch is clean except pipeline output files scheduled for cleanup.
...
```text
### FINAL PR REQUEST: integration -> <default-branch>
...
- Default branch diverged since pipeline start: [Yes / No]
...
8. Create the PR non-interactively with
   `gh pr create --base <default-branch> --head <integration-branch> --title <approved-title> --body-file <approved-body-file>`.
...
if present and commit the removal on the default branch:
...
- if the repository is not post-initial-release, the default branch has been fast-forwarded to
  the integration branch with `git merge --ff-only`, and `hk check -a` passed
- if the repository is post-initial-release, the final PR into the default branch has been
