...
This command does not implement changes, land changes in the default branch, or create pull
...
- if `PIPELINE_STATE.md` does not exist, the command must begin from a clean checkout of the default branch
- on a fresh run, if the current branch is not the default branch, or if the default branch has staged, unstaged, or untracked changes, stop and emit a blocker report instead of starting Phase 0
- do not analyze a feature branch, backup branch, or dirty default-branch checkout as the source tree for a fresh run
...
- if `PIPELINE_STATE.md` is absent, require the current branch to be the default branch and the default branch worktree to be clean before Phase 0 begins
...
2. Resolve the repository default branch using `git rev-parse --abbrev-ref origin/HEAD | sed 's|origin/||'` and record its current SHA as the pipeline base.
3. Create integration branch `integration/pipeline-run-<short-sha>` from the default branch.
...
- Invocation HEAD SHA:
- Invocation Branch:
- Pipeline Base Branch: `<default-branch>`
- Pipeline Base SHA:
- Integration Branch:
- Repository Root:
- Snapshot Timestamp:
...
- Integration discipline: each worktree is reviewed and validated in its own branch, then its approved commit boundary is cherry-picked onto the current integration branch tip. After all worktrees are integrated, the integration branch is rebased onto the default branch if the default branch has moved, then merged.
- Conflict hotspots:
...
- Default final landing method before initial release: `git merge --ff-only` to the default branch
