...
Only fast-forward merges into `dev` are accepted. Use `git merge --ff-only`; never create a merge commit and never fall back to one when fast-forwarding fails.
